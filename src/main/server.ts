...
async function move(from: string,to: string, parent_devID: number) {
    ...
    try{
        var to_stat = await fs.promises.stat(to);
 ...
 * 用于njs调用的主函数
 */
export default async function main(h: FakeNgxResponse, root?: string){
    // APP_ROOT
    root && (APP_ROOT = root);
    ...
    function _error(e: any, sub?: string, code?: number){
        ...
        console.error(new String(e).toString());
    }
